...
Detects tuberculosis spoligotypes in a sequence/read or
  a set of sequencs/reads.
...
Use make (nmake for windows) on the mkfile that targets
  your system. The system namem is added to the end of
  the mkfile's name (ex mkflie.unix is for unix).

You can clean up the .o files
  using `make -f mkfile.os clean`.

## Mac/Linux/BSD (non-static)
...
## Static (Linux/BSD only)

```
make -f mkfile.static
sudo make install
```

## Windows (not tested yet)

Open a developer terminal (view->terminal) in visual
  studio and cd into tbSpolSrc
  (c:\\path\to\freezeTB\programs\tbSpol).
  
```
nmake mkfile.win
```

Then move tbSpol (move tbSpol c:\\path\to\install) to 
...
tbSpol is designed to work on fastq (do not use
...
tbSpol has several modes, which can change the results.
