...
#     in refSTStack (st-01) to make a stack for your
#     structure.
...
|   - Holds a stack of refStruct nodes
\-------------------------------------------------------*/
typedef struct refSTStack
{ /*refSTStack*/
   struct refStruct *nodeST[1024];
...
| Input:
|   - refSTStackST:
|     o Pionter to refSTStack structure to blank
| Output:
|   - Sets:
|     o stackIndexUS to 0
|      o nodeST[0] to 0
\-------------------------------------------------------*/
#define blankRefSTStack(refSTStackST){\
   (refSTStackST)->stackIndexUS = 0;\
   (refSTStackST)->nodeST[0] = 0;\
} /*blankRefSTStack*/
...
| Input:
|   - refTreeST:
|     o Pointer to reference tree to print from
|   - formatC:
|     o Character with flags for format to print out
|       - 0: Print out in markdown format
|       - 1: Print out as tsv
|   - pHeadBl:
|     o 1: print the header
...
|     o table to outFILE
\-------------------------------------------------------*/
static void pRefMapTbl(
   struct refStruct *refTreeST,
   char formatC,
   char pHeadBl,
   FILE *outFILE
){ /*pRefMapTbl*/
   struct refSTStack stackST;
   struct refStruct *nodeST = refTreeST;
   struct refStruct *lastST = 0;

   char *geneHeadStr = "|        geneId        |";
   char *geneSpaceStr = "|:--------------------:|";

   char *totalHeadStr = " TotalMaps  |";
   char *totalSpaceStr = ":----------:|";

   char *primHeadStr = " PrimaryMaps |";
   char *primSpaceStr = ":-----------:|";

   blankRefSTStack(&stackST);

 
   if((formatC) & 1 && pHeadBl)
   { /*If: I am printing out the tsv header*/
      fprintf(
         (outFILE),
         "geneId\tTotalMaps\tPrimaryMaps\n"
      ); /*Print out the tsv header*/
   } /*If: I am printing out the tsv header*/

   else if(pHeadBl)
   { /*Else If: I am printing the markdown header*/
      fprintf(
         (outFILE),
         "%s%s%s\n%s%s%s\n",
          geneHeadStr,
          ...
          totalSpaceStr,
          primSpaceStr
      ); /*Print out the markdown header*/
   } /*Else If: I am printing the markdown header*/

   if(refTreeST->leftST)
      pRefMapTbl(refTreeST->leftST, formatC, 0, outFILE);
   ...
   if((formatC) & 1)
      fprintf(
         ...
         nodeST->idStr,
         nodeST->totalAlnUL,
         nodeST->numPrimaryAlnUL
      ); /*Print out the stats as tsv*/
      ...
      fprintf(
         ...
         nodeST->totalAlnUL,
         nodeST->numPrimaryAlnUL
      ); /*Print out the stats as markdown*/

      /*%-s is for right padding
      ` %.s is for left paddig
      ` %-* set right padding by variable
      ` %.* set left padding by variable
      ` For * the variable comes before the item you
      `  print out
      */

   if(refTreeST->rightST)
      pRefMapTbl(refTreeST->rightST, formatC, 0, outFILE);
...
|     o Pointer to reference tree to print from
|   - stackST:
|     o Stack with the previous links folowed
|   - outFILE:
|     o FILE pointer to file to print to
| Output:
|   - Prints:
|     o tsv with each line starting with the number of
|       mapped reads followed by the genes the read mapped
|       to
\-------------------------------------------------------*/
static void pAmpMapTsv(
   struct refStruct *refTreeST,
   struct refSTStack *stackST,
   FILE *outFILE
){ /*pAmpMapTsv*/
   ...
   if(refTreeST->leftST)
      pAmpMapTsv(refTreeST->leftST, stackST, outFILE);

   /*Put node on stack for links*/
   pushNodeOntoStack(refTreeST, stackST);

   if(refTreeST->link)
      pAmpMapTsv(refTreeST->link, stackST, outFILE);

   else
   { /*Else: At the end of a link*/

      fprintf(
         outFILE,
         "%lu\t%s",
         stackST->nodeST[0]->totalAlnUL,
         stackST->nodeST[0]->idStr
      );

      for(
         usStack = 1;
         usStack <= stackST->stackIndexUS;
         ++usStack
      ){ /*Loop: Print out all genes in the path*/
         fprintf(
            outFILE,
            "\t%lu\t%s",
            stackST->nodeST[usStack]->totalAlnUL,
            stackST->nodeST[usStack]->idStr
         );
      } /*Loop: Print out all genes in the path*/
   ...
   /*Remove the node from the stack of links*/
   popNodeOffStack(stackST); /*Printing is finished*/

   if(refTreeST->rightST)
      pAmpMapTsv(refTreeST->rightST, stackST, outFILE);
