...
#include "generalLib/base10ToNumStr.h"
...
#define def_month_freezeTb 03
#define def_day_freezeTb 19

#define def_freezeTb_minMedianQ 7
#define def_freezeTb_minMeanQ 7

/*Help message variables. These should not clash with
`  tbCons get user input options
*/
#define def_conHelp_freezeTb 32
#define def_printHelp_freezeTb 64

const char *def_prefix_freezeTb = "out";
const char *defPathStr = "";
   /*This is set at complie time with sed. The problem
   `   is that there is no universial way to set this
   `   for all OS's
   */
const char *defAmrDbStr = "freezeTbFiles/who-2023.tsv";
const char *def_graphFlag_freezeTb = "TB";

/*TODO: Add this in
const char *defMiruTblStr = "freezeTbFiles/miruTbl.tsv";
*/

const char *defGeneCoordStr =
...
|       problematic index.
|     o def_tbCon_pHelp if printing the help message
...
|     o def_tbCon_pVersion if printing the version
...
main(
   ...
   *tmpStr = '\';
   ...
   { /*If: I had an error*/
      ...
      if(errI & def_tbCon_pHelp)
      { /*If: I need to print the consensus help message*/
         pHelp_freezeTb(0, 0, stdout);
         exit(0);
      } /*If: I need to print the consensus help message*/

      if(errI & def_tbCon_pVersion)
      { /*If: I need to print the consensus help message*/
         pVersion_freezeTb(stdout);
         exit(0);
      } /*If: I need to print the consensus help message*/

      if(errI & def_tbCon_input_nonNum_Err)
      { /*If: I need to print the consensus help message*/
         errI >>= 8;
   ...
   ^     - Output file for filtered read stats
   ...
   *   - Set up the name for the read stats file
   ...
   /*****************************************************\
   * Main Sec-04 Sub-02:
   *   - Set up the name for the filtered read stats file
   \*****************************************************/
   ...
   tmpStr +=
      ulCpStrDelim(tmpStr, "-depths-filter.tsv", 0, '\0');

   outFILE = fopen(filtStatsStr, "w");
   ...
   *   - Check if I could extract reference name/length
   ...
   if(! refLenUL) refLenUL = defRefLen;
   ...
   *   - Allocate memory for the read stats arrays
   ...
   readMapIAry = calloc(refLenUL, sizeof(int));
   ...
   filt_readMapIAry = calloc(refLenUL, sizeof(int));
   ...
   *   - filter the reads
   ...
   { /*Loop: Process each read in the sam file*/
      ...
      /*Check the Q-scores*/
      if(samST.medianQF < minnMedianQF)
         goto nextSamRead_freezeTb;
      ...
      *   - Build the consensus
      ...
      { /*If: memory error for consensus*/
         freeSamEntryStack(&samST);
         freeGeneCoords(&coordsST, numCoordsI);
         freeAmrStructArray(&amrSTAry);
         freeConBaseAry(&conBaseSTAry, lenRefI);

         free(buffStr);
         free(readMapAry);
         free(filt_readMapAry);
   ...
   */
   ...
   tmpStr +=
      ulCpStrDelim(readStatsStr, graphFlagStr, 0, '\0');
   ...
   *   - Collapse the consensus
   ...
   freeConBaseAry(conBaseAryST, lenReffI);
   ...
   *   - print consensus and AMRs/lineages for consensus
   ...
   { /*Loop: Process each consensus*/
      if(errC)
      { /*If: I had an error*/
         freeSamEntryStack(&samConAryST[iCon]);
         continue;
      } /*If: I had an error*/

      if(samConAryST[iCon].qryIdStr[0] == '\0')
      ...
      { /*If: I had AMRs*/
         pAmrHitLiist(
            samConAryST.qryIdStr,
            amrHitSTList,
            durgStrAry,
            outFILE
         ); /*Print out the AMRs*/

         freeAmrHitSTList(amrHitSTList);
      } /*If: I had AMRs*/
   ...
   *   - Start loop/check file input
   ...
   { /*Loop: Read in the user input*/
      ...
      if(! cStrEql("-sam", argStr, '\0'))
         ...
         *amrDbFileStr = argsStrAry[iArg + 1];

      else if(! cStrEql("-gene-coords", argStr, '\0'))
         *coordFileStr = argsStrAry[iArg + 1];

      /*TODO
      else if(! cStrEql("-miru-tbl", argStr, '\0'))
      {
         ++iArg;
         *miruDbFileStr = argsStrAry[iArg];
      }
      */
      ...
      *     - Consensus min base percent support settings
      ...
      { /*If: The user provided the minimum Q-score*/
         ...
         /*Check for errors*/
         if(tmpStr[0] != '\0')
            return (iArg<<8) | def_tbCon_input_nonNum_Err;
      } /*If: The user provided the minimum Q-score*/
      ...
      { /*If: The user provided the minimum % snp support*/
         ...
         if(settings->minPercSnpF == 0)
            return def_tbCon_input_nonNum_Err;
      } /*If: The user provided the minimum % snp support*/
      ...
      *   - One unique setting for tbAmr
      \**************************************************/
         ...
         *minPercMapF= atof(argsAryStr[iArg + 1]);
      ...
      *   - Read Median/mean q-scores (freezeTb only)
      \**************************************************/
         ...
         *minMedianQF = atof(argsAryStr[iArg + 1]);

      else if(! cStrEql(argStr,"-min-mean-q",'\0'))
         *minMeanQF = atof(argsAryStr[iArg + 1]);
      ...
      *   - ampDepth tsv read depth file column 1 value
      \**************************************************/
         ...
         *graphFlagStr = argsAryStr[iArg + 1];
      ...
      *   - Check for cosensus help message requests
      \**************************************************/
         ...
         return def_tbCon_pHelp | def_conHelp_freezeTb;
      ...
      *   - Check for print help message requests
      \**************************************************/
         ...
         return def_tbCon_pHelp | def_printHelp_freezeTb;
...
pHelp_freezeTb(
   ...
   *     - Print out the median Q-score filter
   ...
   lenHelp += numToStr(&helpStr[lenHelpI], defMinMapQ);
   ...
   lenHelp += numToStr(&helpStr[lenHelpI], defMinLen);
   ...
   *     - Print out the min percent deletion support
   ...
   lenHelp += numToStr(&helpStr[lenHelpI], defMinDepth);
   ...
   lenHelp += numToStr(&helpStr[lenHelpI], defMinBaseQ);
   ...
   lenHelp += numToStr(&helpStr[lenHelpI], defMinInsQ);
