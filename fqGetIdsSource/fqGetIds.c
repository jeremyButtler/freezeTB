...
{ /*main function*/
    ...
    { // If had an error (inputChar has a value )
      ...
      if(
        ...
        fprintf(stdout,"fqGetIds version %u\n",defVersion);
        exit(0);
      } /*Else if the user wanted the version number*/
    ...
    { /*If user takeing all input from stdin*/
        fprintf(
          stderr,
          "Fastq file & filter file are from stdin\n"
        );

        fprintf(
          stderr,
          "Only one file can be passed by stdin\n"
        );
